...
/**
 * Export project to JSON file | 导出项目为 JSON 文件
 * @param {string} id - Project ID to export | 要导出的项目ID
 */
export const exportProject = (id) => {
  ...
  link.href = url
  link.download = `${project.name.replace(/[^a-zA-Z0-9\u4e00-\u9fa5]/g, '_')}_project.json`
...
/**
 * Import project from JSON file | 从 JSON 文件导入项目
 * @param {File} file - JSON file to import | 要导入的 JSON 文件
 * @returns {Promise<{id: string, name: string}>} - Imported project info | 导入的项目信息
 */
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (!data.project || !data.project.canvasData) {
          throw new Error('文件格式不正确，缺少项目数据')
        }

        const sourceProject = data.project
        ...
        const newProject = {
          ...
          updatedAt: now,
          thumbnail: '',
          canvasData: sourceProject.canvasData
        }

        projects.value = [newProject, ...projects.value]
        saveProjects()

        resolve({ id: newId, name: newProject.name })
      } catch (err) {
        reject(err)
      }
    }
