...
export const getModelSizeOptions = (modelKey, quality = 'standard') => {
  ...
  // Handle sizes array - check if items are objects or strings
  if (model.sizes.length > 0 && typeof model.sizes[0] === 'object' && model.sizes[0] !== null) {
    // Format: { label: '1:1 (1024×1024)', key: '1:1' } or { label: '1:1 (1024×1024)', key: '1024x1024' }
    return model.sizes.map(size => {
      // If key contains 'x' (pixel format), try to find a matching SEEDREAM option
      if (size.key.includes('x')) {
        const option = SEEDREAM_SIZE_OPTIONS.find(o => o.key === size.key) ||
                       SEEDREAM_4K_SIZE_OPTIONS.find(o => o.key === size.key)
        return option || { label: size.label, key: size.key }
      }
      // For ratio format like '1:1', return as-is
      return { label: size.label, key: size.key }
    })
  }

  // Legacy string array format | 遗留字符串数组格式
