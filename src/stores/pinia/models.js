...
export const useModelStore = defineStore('model', () => {
  ...
  // 按渠道存储的自定义模型 | 结构: { 'openai': [{key, label}], 'xiaodao': [{key, label}] }
  ...
  const setApiKeyByProvider = (provider, apiKey) => {
    ...
    if (key) {
      apiKeysByProvider.value[LOCKED_PROVIDER] = key
    } else {
  ...
  // 获取图片端点
  const getImageEndpoint = (modelKey) => {
    // Gemini 图片模型使用特殊的 generateContent 端点
    if (modelKey && modelKey.includes('gemini')) {
      const endpoint = providerConfig.value.endpoints?.geminiImage || '/v1beta/models/{model}:generateContent'
      const modelName = modelKey || 'gemini-2.5-flash-image'
      return `${currentBaseUrl.value}${endpoint.replace('{model}', modelName)}`
    }
