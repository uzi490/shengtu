  ...
  <div class="image-node-wrapper" @mouseenter="showActions = true; showHandleMenu = true" @mouseleave="showActions = false; showHandleMenu = false">
    ...
    <div
      ...
      <div class="p-3">
        ...
        <div 
          v-else-if="data.url" 
          class="rounded-xl overflow-hidden relative w-full"
...
const handleDownload = async () => {
  if (props.data.url) {
    try {
      // 如果是 blob URL，需要用 fetch 获取后下载
      if (props.data.url.startsWith('blob:')) {
      ...
      } else if (isBase64ImageDataURL(props.data.url)){
        debugger
        const response = await fetch(props.data.url)
        const blob = await response.blob()
