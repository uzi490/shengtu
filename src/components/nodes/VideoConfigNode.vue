...
const localResolution = ref(props.data?.resolution || '768P')
...
const resolutionOptions = computed(() => {
  ...
  if (config?.resolutions && config.resolutions.length > 0) {
    return config.resolutions.map(r => ({ label: r.toUpperCase(), key: r.toUpperCase() }))
  }
  return [
    { label: '720P', key: '720P' },
    { label: '768P', key: '768P' },
...
const handleModelSelect = (key) => {
  ...
  if (config?.defaultParams?.resolution) {
    localResolution.value = config.defaultParams.resolution.toUpperCase()
